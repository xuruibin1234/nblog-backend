...
public class UserController {
    ...
    BaseActionResponse<User> findUsernameByEmail(@RequestBody User user) {
        BaseActionResponse<User> findUsernameByEmail = userService.findUsernameByEmail(user);
        if (findUsernameByEmail.getCode() == 200) {
            User data = findUsernameByEmail.getData();
    ...
    BaseActionResponse<User> findPasswordByEmail(@RequestBody User user) {
        try {
            ...
            if (findUsernameByEmail.getCode() == 200) {
                ...
                String username = data.getUsername();
                String email = user.getEmail();
                SendEmailDto sendEmailDto = new SendEmailDto("17815708315@163.com",email, "nblog找回密码邮箱验证", "您好，您正在找回登录用户名为 "+ username+"的博客园账号的密码" +
                        "\n您的验证码为: "+body+"\n如果不是本人操作，请忽略。\n \n nblog - 博客");
                emailService.sendEmail(sendEmailDto);
            }
            return findUsernameByEmail;
